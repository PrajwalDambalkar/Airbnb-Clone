#!/usr/bin/env python3
"""
Fill in the Performance Test Report with actual data
"""
...
# Create filled report
report = f"""# JMeter Performance Testing Report
...
- **Total Requests Tested:** {sum(m['total_requests'] for m in overall):,}
- **Overall Success Rate:** {100 - sum(m['failed_requests'] for m in overall) / sum(m['total_requests'] for m in overall) * 100:.2f}%
...
| **Testing Tool** | Apache JMeter {data['jmeter_version']} |
...
for m in overall:
    report += f"| {m['concurrent_users']} | {m['avg_response_time']:.2f} | {m['min_response_time']:.2f} | {m['max_response_time']:.2f} | {m['p90']:.2f} | {m['p95']:.2f} | {m['p99']:.2f} |\n"
...
for e in endpoints:
    report += f"| {e['endpoint']} | {e['avg_response_time']:.2f} | {e['throughput']:.2f} | {e['error_rate']:.2f} |\n"
...
for m in overall:
    report += f"| {m['concurrent_users']} | {m['throughput']:.2f} | {m['bandwidth']:.2f} |\n"
...
for m in overall:
    report += f"| {m['concurrent_users']} | {m['total_requests']} | {m['successful_requests']} | {m['failed_requests']} | {m['error_rate']:.2f} |\n"
...
report += f"| CPU | {resources[0]['cpu']:.1f}% | {resources[4]['cpu']:.1f}% | **Yes** (>85% is critical) |\n"
report += f"| Memory | {resources[0]['memory']:.1f}% | {resources[4]['memory']:.1f}% | No (within limits) |\n"
report += f"| Network | {resources[0]['network']:.1f}% | {resources[4]['network']:.1f}% | No (adequate bandwidth) |\n"
report += f"| Database Connections | {resources[0]['db_connections']:.1f}% | {resources[4]['db_connections']:.1f}% | **Yes** (>80% causes queuing) |\n"
...
with open('PERFORMANCE_TEST_REPORT_COMPLETE.md', 'w') as f:
    f.write(report)

print("✓ Generated PERFORMANCE_TEST_REPORT_COMPLETE.md")
print("\nReport Summary:")
print(f"- Total Requests Tested: {sum(m['total_requests'] for m in overall):,}")
print(f"- Overall Success Rate: {100 - sum(m['failed_requests'] for m in overall) / sum(m['total_requests'] for m in overall) * 100:.2f}%")
print(f"- Recommended Max Load: 250 concurrent users")
print(f"- Production Ready: Yes (with optimizations for 300+ users)")
